...
const { getProvider, validateFilePath, validate_uint256, getProxyAdminAddress, getDeploymentConfig, selectAccount, inquireAddress } = require('../utils');
...
module.exports = async function (params, { web3, artifacts, network }) {
  ...
  const proposal = new Proposal(proposal_data, {
    web3,
    artifacts,
    proxyAdminAddress: getProxyAdminAddress(DC) || await inquireAddress(web3, 'proxy_admin', 'ProxyAdmin address'),
